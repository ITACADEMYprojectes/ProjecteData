...
-- Miro el dataset
SELECT *
FROM Tourist_Accommodation27042026
LIMIT 10;

DESCRIBE Tourist_Accommodation27042026;
...
-- Si necesito borrar la tabla: DROP TABLE copy_ta27042026
CREATE TABLE IF NOT EXISTS copy_ta27042026 AS
SELECT *
FROM Tourist_Accommodation27042026;
...
-- Si necesito borrar la tabla: DROP TABLE copy_ta27042026_latest
CREATE TABLE IF NOT EXISTS copy_ta27042026_latest AS
SELECT *
FROM copy_ta27042026 t1
WHERE STR_TO_DATE(insert_date, '%d/%m/%Y') = (
   SELECT MAX(STR_TO_DATE(insert_date, '%d/%m/%Y')) -- mas reciente
   FROM copy_ta27042026 t2
   WHERE t1.apartment_id = t2.apartment_id
);

-- Miro el output:
SELECT *
FROM copy_ta27042026_latest
LIMIT 10;

-- Compruebo que el numero de filas borradas sea justo:
SELECT COUNT(*) FROM copy_ta27042026;
-- 7001 filas totales
-- 268 duplicados
-- Espero encontrar 6733 filas en la nueva tabla copy_ta27042026_latest (7001-268)

SELECT COUNT(*) FROM copy_ta27042026_latest;
...
DROP TABLE copy_ta27042026;
RENAME TABLE copy_ta27042026_latest TO copy_ta27042026;
...
CREATE TABLE group_medians2 AS
...
CREATE TABLE median_price_by_group2 AS
SELECT 
    c.city,
    c.room_type,
    c.n_observaciones,
    m.median_price
FROM group_counts2 c
JOIN group_medians2 m
    ON c.city = m.city
   AND c.room_type = m.room_type;
   
SELECT *
FROM median_price_by_group22;

-- creo la tabla filtrada: tengo solo los grupos city + room_type con un numero de observaciones > 10.
CREATE TABLE median_price_valid_groups2 AS
SELECT *
FROM median_price_by_group2
WHERE n_observaciones >= 10;

-- Check del output:
SELECT *
FROM median_price_valid_groups2
ORDER BY n_observaciones ASC, city, room_type;

-- hago la imputacion de los precios:
SET SQL_SAFE_UPDATES = 0;

UPDATE copy_ta27042026 AS t
JOIN median_price_valid_groups2 AS m
    ON t.city = m.city
